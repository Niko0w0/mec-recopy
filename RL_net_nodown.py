...
class ENV():
    ...
    def reset(self):
        ...
        for i in range(self.UEs):
            ...
            observation = np.hstack((observation, servers_cap, uplink))
    ...
    def step(self, observation, actions_prob, is_prob=True, is_compared=True):
        ...
        for i in range(self.UEs):
            ...
            task_size, task_cpu_cycle, local_comp, servers_cap, uplink = \
                observation[i][0], observation[i][1], observation[i][2], observation[i][3:3+self.MECs], observation[i][3+self.MECs:3+self.MECs*2]
            ...
            tr_time = (percen * task_size) / uplink[target_server] + self.discount * ( percen * task_size)
            tr_energy = (self.tr_energy * percen * task_size) / uplink[target_server] + self.discount * (self.tr_energy * percen * task_size)
            ...
            mec_only_tr_time = task_size / uplink[target_server] + self.discount * task_size
            mec_only_tr_energy = self.tr_energy * task_size / uplink[target_server] + self.discount * self.tr_energy * task_size
            ...
            random_tr_energy = (self.tr_energy * percen_ran * task_size) / uplink[mec_ran] + self.discount * (self.tr_energy * percen_ran * task_size)
            ...
            observation_ = np.hstack((observation_, servers_cap, uplink))
